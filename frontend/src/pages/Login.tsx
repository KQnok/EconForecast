  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'var(--bg)', fontFamily: 'DM Sans, sans-serif' }}>
      <div className="hidden lg:flex flex-col justify-between p-12 w-[420px] flex-shrink-0" style={{ background: 'var(--primary)' }}>
        ...
        <div>
          <p className="text-white/60 text-sm mb-2">Инструмент анализа</p>
          <h2 className="text-white text-3xl font-bold leading-snug mb-6">Прогнозируйте эффективность проектов с точностью</h2>
          <div className="flex flex-col gap-3">
            {['NPV, IRR, срок окупаемости', 'Анализ сценариев и чувствительности', 'Моделирование Монте-Карло'].map(f => (
      ...
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="w-full max-w-sm animate-fade-up">
          <h1 className="text-2xl font-bold mb-1" style={{ color: 'var(--text)' }}>Вход в систему</h1>
          <p className="text-sm mb-8" style={{ color: 'var(--text-muted)' }}>
            Нет аккаунта?{' '}
            <Link to="/register" style={{ color: 'var(--primary)' }} className="font-medium hover:underline">Зарегистрироваться</Link>
          </p>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <button type="submit" className="btn btn-primary w-full justify-center mt-2" disabled={loading}>
              {loading ? 'Вход...' : 'Войти'}{!loading && <ArrowRight size={15} />}
            </button>
          </form>
        </div>
      </div>
